...
const Ham = () => {
        ...
        let endTime = workHistory[index].wctime;
         if(gubun == "start"){
          ...
          if(startTime != "기록없음" && endTime != "기록없음"){
            ...
            return <td class="rowTimeStamp" bgcolor={todayIndex == index+1 ? "#E6FFE6" : ""}><span className="timeGapFont">{timeGap > 0 ? "+"+timeGap : timeGap}</span></td>
          }else{
            timeGapArray.push("X");
    ...
    for(let index=0; index < timeGapArray.length ; index++){
      if(timeGapArray[index] == "X"){
        ...
        if(index == 0){
          ...
          let sum = timeGapArray[index] + timeGapArray[index-1];
  ...
  useEffect(() => {
    ...
    const getExceptionHandling = async () => {
      ...
      console.log(" getExceptionHandling > ", result.data)
  ...
  const hanldeAttendanceException = async (gubun, index) => {    
    ...
    console.log(" exceptionHandling >> ", exceptionHandling);
